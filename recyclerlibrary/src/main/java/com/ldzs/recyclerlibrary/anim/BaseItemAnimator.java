...
/**
 * code come from github:RecyclerViewAninmator
 */
public abstract class BaseItemAnimator extends SimpleItemAnimator {
    ...
    public void runPendingAnimations() {
        ...
        // First, remove stuff
        int size = mPendingRemovals.size();
        for (int i = 0; i < size; i++) {
            ViewHolder viewHolder = mPendingRemovals.get(i);
            doAnimateRemove(viewHolder, i);
        }
        ...
        if (additionsPending) {
            ...
            Runnable adder = new Runnable() {
                public void run() {
                    ...
                    for (int i = 0; i < size; i++) {
                        ViewHolder viewHolder = additions.get(i);
                        doAnimateAdd(viewHolder, i);
                    }
                    additions.clear();
    ...
    protected abstract void animateRemoveImpl(final RecyclerView.ViewHolder holder, int index);

    protected abstract void animateAddImpl(final RecyclerView.ViewHolder holder, int index);
    ...
    private void doAnimateRemove(final RecyclerView.ViewHolder holder, int index) {
        if (holder instanceof AnimateViewHolder) {
    ...
    private void doAnimateAdd(final RecyclerView.ViewHolder holder, int index) {
        if (holder instanceof AnimateViewHolder) {
